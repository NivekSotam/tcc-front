...
import { fetchItemCategoriaData, fetchItemData } from "./helpers/api";
...
const ListagemItemCategoria = () => {
  const [searchType, setSearchType] = useState("categoria");
  ...
  const handleMenuItemClick = (type: string) => {
    ...
    setSearchTerm("");
    setCurrentPage(1);
    setButtonText(type === "nome" ? "Nome" : "Categoria");
  };
  ...
  const fetchDataFromApi = useCallback(async () => {
    try {
      ...
      const fetchedData = await fetchItemCategoriaData({
        nome,
        userToken,
        currentPage,
        itemsPerPage,
...
export default ListagemItemCategoria;
